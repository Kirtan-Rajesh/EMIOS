...
from app.models.schemas import (
    ...
    ChatRequest, ChatResponse, AssessmentReport
)
...
from app.agents.workflow import run_agent_negotiation, run_full_assessment
...
@router.post("/assess", response_model=AssessmentReport)
def run_assessment():
    """
    Orchestrates the full 12-agent Master Orchestrator pipeline (Discovery through
    Report Generation) over the current Digital Twin graph and returns the complete
    ...
    try:
        ...
        final_state = run_full_assessment(nodes_dict, edges_dict)

        cloud_readiness = final_state.get("cloud_readiness", {})

        return AssessmentReport(
            assessment_summary=final_state.get("report", {}).get("assessment_summary", {}),
            migration_readiness=cloud_readiness,
            complexity=final_state.get("complexity", {}),
            risk=final_state.get("enterprise_risk", {}),
            cloud_recommendation=cloud_readiness,
            migration_plan={
        ...
        raise WorkflowException(f"Full assessment orchestration failed: {e}")
